# Larger-output projection runtime profile for the DeepONet helper benchmark.
# Run: julia --project=benchmarks/deeponet benchmarks/deeponet/profile_larger_outputs.jl
...
const SCENARIO = larger_profile_scenario()
...
seconds_per_call(elapsed, calls) = elapsed / calls
...
open(joinpath(OUT, "larger_projection_profile.csv"), "w") do io
    ...
    for K in GRIDS
        rng = MersenneTwister(120_000 + K)
        data, x, w = sample_heat_operator(NSAMPLES, K, rng = rng)
        p = init_deeponet(seed = 130_000 + K)
        ...
        heat_correction_contexts(data[1:min(2, length(data))], w,
        ...
        uncached_time = best_time() do
            for _ in 1:REPEATS, d in data
                ...
                res = project(c, pred)
                StructPINN.issuccess(res) ||
                    error("uncached larger profile failed")
            end
        end
        ...
        construct_time = best_time() do
            contexts = heat_correction_contexts(data, w, mode = :full,
                                                cached = true)
            length(contexts) == NSAMPLES ||
                error("context count mismatch")
        end
        ...
        cached_time = best_time() do
            for _ in 1:REPEATS, (d, c) in zip(data, cached_constraints)
                ...
                res = project(c, pred)
                StructPINN.issuccess(res) ||
                    error("cached larger profile failed")
            end
        end
...
println("wrote larger_projection_profile.csv to ", OUT)
